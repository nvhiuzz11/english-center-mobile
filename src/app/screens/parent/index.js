...
import {ModalParentInfo} from './components/modalParentInfo';
...
export const ParentScreen = props => {
  ...
  const [prarents, setParents] = useState([]);
  ...
  const fetchParent = async () => {
    try {
      const response = await authAxios.get('api/parent-connected');
      if (response.status === StatusCodes.OK) {
        console.log('fetchParent res  :', response.data);
        setParents(response.data);
      }
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
      console.error('fetchParent error', error);
    }
  };
    ...
    try {
      ...
      if (res.status === StatusCodes.OK) {
        ...
        Toast.show({
          type: 'success',
          props: {
            title: translate('Added associated parent successfully'),
          },
        });
        setIsLoading(false);
      }
      ...
      Toast.show({
        type: 'error',
        props: {
          title: translate('Add parent link failed'),
        },
      });
    }
  };
    ...
    try {
      ...
      if (res.status === StatusCodes.OK) {
        ...
        Toast.show({
          type: 'success',
          props: {
            title: translate('Deleted associated parent successfully'),
          },
        });
        setIsLoading(false);
      }
      ...
      Toast.show({
        type: 'error',
        props: {
          title: translate('Delete associated parent failed'),
        },
      });
    }
  };
    ...
    <Swipeable
      ...
      <TouchableOpacity
        ...
        <View style={styles.leftContainer}>
          <Avatar
            size="large"
            source={getAvatar(ROLE.PARENT, item.gender)}
            style={styles.avatar}
          />
      ...
      <View style={styles.container}>
        <View>
          <Text style={styles.header}>
            {translate('List of parents linked to you')}
          </Text>
          {/* {prarents.length < 0  &&} */}
          <FlatList
            data={prarents}
