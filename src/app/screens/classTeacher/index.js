...
import {formatDateFromISO, sortStudentsByFirstName} from '@utils/input';
...
export const ClassTeacherScreen = () => {
  ...
  const fetchClasses = async () => {
    try {
      const response = await authAxios.get('api/class-by-teacher');
      if (response.status === StatusCodes.OK) {
        console.log('fetchClasses res  class :', response.data);
        const classesData = response.data;

        const sortedClassData = sortStudentsByFirstName(classesData);

        setClasses(sortedClassData);
      }
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  const renderItem = ({item, index}) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate(SCREEN_NAME.DETAIL_TEACHER_CLASS, {
            payload: {
              classData: item,
            },
          });
    ...
    <Container>
      ...
      <View style={styles.container}>
        <Text style={styles.descriptiopn}>
          {translate('Teaching')}: {classes?.length || '0'}{' '}
          {translate('classes')}
        </Text>
